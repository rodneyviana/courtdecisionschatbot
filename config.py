...
def edit_config(config):
    ...
    fields = {
        ...
        "azure_open_ai_base_url": gr.components.Textbox(label="Azure/OpenAI Base URL", value=config.azure_open_ai_base_url, info="Always use https://api.openai.com/v1/chat/completions for OpenAI. For azure it looks like https://<your-resource-name>.openai.azure.com/"),
        ...
        "azure_doc_ai_endpoint": gr.components.Textbox(label="Azure DocAI Endpoint", value=config.azure_doc_ai_endpoint, info="Azure Document AI endpoint. If blank it will use local conversion. WARNING: If Document AI instance uses free tier, only the first 2 pages are converted, so instead of free instance use the local conversion. The url looks like https://<your-resource-name>.cognitiveservices.azure.com/"),
        ...
        "open_ai_temperature": gr.components.Slider(minimum=0, maximum=1, step=0.01, label="OpenAI Temperature", value=config.open_ai_temperature, info="Controls the randomness of responses. Lower values produce more deterministic and conservative outputs, while higher values lead to more diverse and creative results. If you're unsure, start with 0.5."),
        "open_ai_top_p": gr.components.Slider(minimum=0, maximum=1, step=0.01, label="OpenAI Top P", value=config.open_ai_top_p, info="Determines the range of probabilities considered when generating responses. Lower values (e.g., 0.1) make outputs more focused and predictable, while higher values (e.g., 0.9) allow for more diverse and creative results. If using the temperature, you may set this to 0 for simplicity."),
        "open_ai_frequency_penalty": gr.components.Slider(minimum=0, maximum=1, step=0.01, label="OpenAI Frequency Penalty", value=config.open_ai_frequency_penalty, info="Controls the penalty for using common words. Higher values reduce the likelihood of common words, while lower values allow for more frequent usage. If you're unsure, start with 0."),
        "open_ai_presence_penalty": gr.components.Slider(minimum=0, maximum=1, step=0.01, label="OpenAI Presence Penalty", value=config.open_ai_presence_penalty, info="Controls the penalty for using new words. Higher values reduce the likelihood of new words, while lower values allow for more frequent usage. If you're unsure, start with 0."),
        "gradio_share": gr.components.Checkbox(label="Share on the Internet", value=config.gradio_share, info="Check this box to make the chatbot available via the Internet. Uncheck it to make local only. Antivirus may identify it as a false positive virus."),
        "gradio_port": gr.components.Textbox(label="Gradio Port", value=config.gradio_port, info="Port to run the Gradio server. Normally 7860. If you are running multiple instances, change this to a different port."), 
